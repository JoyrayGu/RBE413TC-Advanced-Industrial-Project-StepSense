...
static uint8_t _vl53l5cx_poll_for_answer(VL53L5CX_Configuration *p_dev,
  ...
  do {
    status |=
        VL53L5CX_RdMulti(&(p_dev->platform), address, p_dev->temp_buffer, 4);
...
/*
 * Inner function, not available outside this file. This function is used to
 * program the output using the macro defined into the 'vl53l5cx_platform.h'
 * file.
 */
...
uint8_t vl53l5cx_calibrate_xtalk(VL53L5CX_Configuration *p_dev,
  ...
  if (((reflectance < (uint16_t)1) || (reflectance > (uint16_t)99)) ||
    ...
    status |= VL53L5CX_WrMulti(&(p_dev->platform), 0x2c28, p_dev->temp_buffer,
    ...
    do {
      status |=
          VL53L5CX_RdMulti(&(p_dev->platform), 0x0, p_dev->temp_buffer, 4);
  ...
  status |= VL53L5CX_WrMulti(&(p_dev->platform), 0x2fb8, p_dev->temp_buffer,
  ...
  (void)memcpy(&(p_dev->xtalk_data[VL53L5CX_XTALK_BUFFER_SIZE - (uint16_t)8]),
               footer, sizeof(footer));

  /* Reset default buffer */
  status |=
      VL53L5CX_WrMulti(&(p_dev->platform), 0x2c34, p_dev->default_configuration,

 ...
 * two different modes to save power consumption when is the device is not used:
 * - VL53L8CX_POWER_MODE_SLEEP: This mode retains the firmware and the
 * configuration. It is recommended when the device needs to quickly wake-up.
 * - VL53L8CX_POWER_MODE_DEEP_SLEEP: This mode clears all memory, by consequence
 * the firmware, the configuration and the calibration are lost. It is
 * recommended when the device sleeps during a long time as it consumes a very
 * low current consumption. Both modes can be changed using function
...
#define VL53L8CX_MAX_RESULTS_SIZE                                              \
  (40U + L8CX_AMB_SIZE + L8CX_SPAD_SIZE + L8CX_NTAR_SIZE + L8CX_SPS_SIZE +     \
...
#ifndef VL53L8CX_DISABLE_SIGNAL_PER_SPAD
  uint32_t
      signal_per_spad[(VL53L8CX_RESOLUTION_8X8 * VL53L8CX_NB_TARGET_PER_ZONE)];
#endif

  /* Sigma of the current distance in mm */
#ifndef VL53L8CX_DISABLE_RANGE_SIGMA_MM
  uint16_t
      range_sigma_mm[(VL53L8CX_RESOLUTION_8X8 * VL53L8CX_NB_TARGET_PER_ZONE)];
#endif
...
#ifndef VL53L8CX_DISABLE_TARGET_STATUS
  uint8_t
      target_status[(VL53L8CX_RESOLUTION_8X8 * VL53L8CX_NB_TARGET_PER_ZONE)];
#endif
...
/**
 * @brief This function is used to enable or disable the synchronization pin.
 * When it is enabled, the sensor waits an interrupt on B1 pin to start the next
 * measurement. It is useful for multi-devices synchronization. By default the

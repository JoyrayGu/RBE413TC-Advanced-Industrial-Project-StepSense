...
static uint8_t _vl53l8cx_poll_for_answer(VL53L8CX_Configuration *p_dev,
  ...
  do {
    status |=
        VL53L8CX_RdMulti(&(p_dev->platform), address, p_dev->temp_buffer, 4);
...
/*
 * Inner function, not available outside this file. This function is used to
 * program the output using the macro defined #include "vl53l8cx_platform.h"'
 * file.
 */

static uint8_t _vl53l8cx_program_output_config(VL53L8CX_Configuration *p_dev) {
  ...
  uint32_t output[] = {0x0000000DU, 0x54000040U, 0x9FD800C0U, 0x9FE40140U,
                       0x9FF80040U, 0x9FFC0404U, 0xA0FC0100U, 0xA10C0100U,
                       0xA11C00C0U, 0xA1280902U, 0xA2480040U, 0xA24C0081U,
                       0xA2540081U, 0xA25C0081U, 0xA2640081U, 0xA26C0084U,
                       0xA28C0082U};
  ...
  for (i = 0; i < (uint32_t)(sizeof(output) / sizeof(uint32_t)); i++) {
    ...
    if (((uint8_t)bh_ptr->type >= (uint8_t)0x1) &&
      ...
      if ((bh_ptr->idx >= (uint16_t)0x54d0) &&
        ...
        bh_ptr->size =
            (uint8_t)(resolution * (uint8_t)VL53L8CX_NB_TARGET_PER_ZONE);
      }
...
uint8_t vl53l8cx_calibrate_xtalk(VL53L8CX_Configuration *p_dev,
  ...
  if (((reflectance < (uint16_t)1) || (reflectance > (uint16_t)99)) ||
    ...
    status |= VL53L8CX_WrMulti(&(p_dev->platform), 0x2c28, p_dev->temp_buffer,
                               (uint16_t)sizeof(VL53L8CX_CALIBRATE_XTALK));
    status |= _vl53l8cx_poll_for_answer(p_dev, VL53L8CX_UI_CMD_STATUS, 0x3);
  ...
  status |= VL53L8CX_WrMulti(&(p_dev->platform), 0x2fb8, p_dev->temp_buffer,
  ...
  (void)memcpy(&(p_dev->xtalk_data[VL53L8CX_XTALK_BUFFER_SIZE - (uint16_t)8]),
...
uint8_t vl53l8cx_get_caldata_xtalk(VL53L8CX_Configuration *p_dev,
...
uint8_t vl53l8cx_set_caldata_xtalk(VL53L8CX_Configuration *p_dev,
                                   uint8_t *p_xtalk_data) {
  uint8_t resolution, status = VL53L8CX_STATUS_OK;
...
uint8_t vl53l8cx_get_xtalk_margin(VL53L8CX_Configuration *p_dev,
...
uint8_t vl53l8cx_set_xtalk_margin(VL53L8CX_Configuration *p_dev,
